...
SELECT distinct(DATE), sum(cast(new_cases as int)) as new_cases, sum(cast(new_deaths as int)) as new_deaths, sum(cast(new_deaths as float))/ sum(NULLIF(cast(new_cases as float), 0)) *100  AS 'Global Death %'
FROM [Portfolio project1]..[covid deaths]
where continent is not null
group by date 
...
select deaths.continent, deaths.location, deaths.date, cast(deaths.population as int) as T_pop, cast(vaccine.new_vaccinations as int),
sum(cast(vaccine.new_vaccinations as int)) OVER (Partition by deaths.location order by deaths.location,deaths.date)
from [Portfolio project1]..[covid deaths] as deaths
join [Portfolio project1]..[covid vaccinations] as vaccine on
deaths.continent = vaccine.continent and deaths.date = vaccine.date
